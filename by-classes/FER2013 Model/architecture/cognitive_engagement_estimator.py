...
class CognitiveEngagementEstimator:
    def __init__(
        ...
        self.legacyEstimator = cog.CognitiveEngagementEstimator()
    ...
    def infer(self, frame, face: Face, landmarks: Landmarks) -> CognitiveResult:
        attention_score, details, _, _ = self.legacyEstimator.update(frame.data)
        attention = AttentionResult(score=float(attention_score), details=details)
