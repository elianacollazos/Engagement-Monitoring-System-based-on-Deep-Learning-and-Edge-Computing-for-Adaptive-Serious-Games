...
class FERPreprocessor:
    """FER2013 preprocessor used by FER2013EmotionRecognition."""
    ...
    def preprocess(self, face_image):
        return ProcessedFrame(self.transform(face_image).unsqueeze(0))

    def __call__(self, face_image):
        return self.preprocess(face_image)
...
class FER2013EmotionRecognition(EmotionRecognition):
    """FER2013 specialization of EmotionRecognition."""
    ...
    def __init__(
        ...
        if model is None:
            model, self.metadata = load_model(model_path, model_type)
        else:
            self.metadata = {}
        ...
        super().__init__(model, preprocessor or FERPreprocessor())

    def infer(self, input: ProcessedFrame) -> EmotionResult:
        processed = input if isinstance(input, ProcessedFrame) else self.preprocessor(input)
        face_tensor = processed.data.to(self.device)
        prediction = predict_emotion(self.model, face_tensor, self.confidence_threshold)
        emotion_index = int(prediction["emotion"])

        return EmotionResult(
            emotion=FER2013_EMOTION_LABELS[emotion_index],
            emotion_index=emotion_index,
            confidence=float(prediction["confidence"]),
            is_reliable=bool(prediction["is_reliable"]),
...
def preprocess_face(face_image):
    ...
    return FERPreprocessor().preprocess(face_image).data
